...
// If the target is a raw IP without protocol, prepend http:// (default to http)
if (!targetArg.startsWith('https://') && !targetArg.startsWith('http://')) {
    const ipRegex = /^(?:\d{1,3}\.){3}\d{1,3}$/;
    if (ipRegex.test(targetArg)) {
        // Assume HTTP when only IP is provided
        targetArg = 'http://' + targetArg;
    } else {
        console.error('Error: target must start with https://, http://, or be a raw IP address');
        process.exit(1);
    }
}

const url = new URL(targetArg);
    ...
    const netSocket = net.connect(Number(proxyPort), proxyHost, () => {
        netSocket.once('data', () => {
            tlsSocket = tls.connect({
                ...
                secure: true,
                minVersion: 'TLSv1.2',
                // maxVersion removed for compatibility with older Node.js versions
